    ...
    >
      <div
        ...
        :class="[
          ...
          clientToast.variant === 'success' ? 'border-green-200 border-l-4 border-l-green-500' :
          clientToast.variant === 'error' ? 'border-red-200 border-l-4 border-l-red-500' :
          'border-blue-200 border-l-4 border-l-blue-500'
        ]"
      >
        <svg v-if="clientToast.variant === 'success'" class="w-5 h-5 text-green-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
        </svg>
        <svg v-else-if="clientToast.variant === 'error'" class="w-5 h-5 text-red-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <span class="text-sm text-gray-700">{{ clientToast.message }}</span>
        <button @click="clientToast.show = false" class="flex-shrink-0 text-gray-400 hover:text-gray-700 transition p-0.5 ml-auto">
      ...
      <div v-if="state.isOpen" class="fixed inset-0 z-50">
        ...
        <div class="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col">
          ...
          <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            ...
            <div v-for="item in items" :key="item.id" class="bg-gray-50 border border-gray-200 rounded-lg p-3 flex gap-3">
              <!-- Image -->
              <div class="w-12 h-12 rounded-md bg-gray-200 flex-shrink-0 overflow-hidden">
                <img v-if="item.image" :src="getImageUrl(item.image)" :alt="item.name" class="w-full h-full object-cover" />
                <svg v-else class="w-full h-full text-gray-300 p-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex-1 min-w-0 flex flex-col justify-between">
                <div class="flex justify-between items-start">
                  <div>
                    <p class="font-semibold text-gray-900 text-sm line-clamp-1 leading-tight">{{ item.name }}</p>
                    <p class="text-[10px] text-gray-500 mt-0.5">{{ item.type === 'kustom_box' ? 'Snack Box' : 'Kue Satuan' }}</p>
                  </div>
                  <button @click="removeItem(item.id)" class="text-red-400 hover:text-red-600 transition p-1 flex-shrink-0 ml-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </div>
                ...
                <div class="flex items-center justify-between mt-1.5">
                  <div class="flex items-center bg-gray-50 rounded-md border border-gray-200">
                    <button @click="updateQty(item.id, item.qty - 1)" class="px-2 py-0.5 hover:bg-gray-200 transition rounded-l-md text-gray-600">
                      <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" /></svg>
                    </button>
                    <span class="px-2 py-0.5 font-semibold text-xs min-w-[24px] text-center text-gray-700">{{ item.qty }}</span>
                    <button @click="updateQty(item.id, item.qty + 1)" class="px-2 py-0.5 hover:bg-gray-200 transition rounded-r-md text-gray-600">
                      <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                    </button>
                  </div>
                  <p class="font-bold text-blue-600 text-xs">Rp {{ formatNumber(item.price * item.qty) }}</p>
                </div>
              </div>
            </div>
            <!-- Formulir Pemesan (Moved into the scrollable area) -->
            <div v-if="items.length > 0" class="border-t border-gray-200 pt-5 mt-2 space-y-3">
              ...
              <div>
                ...
                <input v-model="customerName" type="text" placeholder="Contoh: Budi Santoso" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 transition text-sm" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">No. WhatsApp / Telepon</label>
                <input v-model="customerPhone" type="tel" placeholder="Contoh: 08123456789" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 transition text-sm" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Tanggal & Jam Pengambilan</label>
              ...
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Lokasi / Alamat Pengiriman</label>
                <textarea v-model="customerLocation" rows="2" placeholder="Contoh: Jl. Raya No. 123, Jakarta" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 transition text-sm resize-none"></textarea>
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Catatan (opsional)</label>
                <textarea v-model="notes" rows="2" placeholder="Misal: tambahan pesan atau permintaan khusus" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 transition text-sm resize-none"></textarea>
              </div>
            </div>
          </div>

          <!-- Fixed Footer (Total & Checkout Buttons only) -->
          <div v-if="items.length > 0" class="border-t border-gray-200 px-6 py-4 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10 relative space-y-3">
            <div class="space-y-1">
              ...
              <div class="flex justify-between text-sm mt-1 border-t border-dashed border-gray-200 pt-1">
                <span class="text-gray-500 text-xs">Sisa Bayar</span>
                <span class="font-bold text-gray-700 text-xs">Rp {{ formatNumber(totalPrice - dpPrice) }}</span>
              </div>
            </div>
...
const showToast = (variant, message) => {
  ...
  setTimeout(() => { clientToast.value.show = false }, 4000)
}

const sendToWhatsApp = () => {
  if (items.value.length === 0) { showToast('error', 'Pilih produk dulu!'); return }
  if (!customerName.value.trim()) { showToast('error', 'Silakan masukkan nama lengkap'); return }
  if (!customerPhone.value.trim()) { showToast('error', 'Silakan masukkan nomor WhatsApp'); return }

  const phoneRegex = /^(\+62|0)[0-9]{9,12}$/;
  if (!phoneRegex.test(customerPhone.value.trim())) {
    showToast('error', 'Format nomor telepon tidak valid (gunakan format: 0812xxx atau +62812xxx)');
    return;
  }

  if (!pickupDate.value) { showToast('error', 'Silakan pilih tanggal pengambilan'); return }
  if (!customerLocation.value.trim()) { showToast('error', 'Silakan masukkan lokasi/alamat pengambilan'); return }
  ...
  .then((response) => {
    // API trigger succeeded, now open WhatsApp!
    ...
    const boxItems = itemsArray.filter(i => i.type === 'kustom_box')
  ...
  .catch((error) => {
    ...
    if (error.response) {
      if (error.response.status === 422 && error.response.data?.errors) {
        ...
        const pesan = Object.values(error.response.data.errors).flat().join('. ');
        showToast('error', pesan);
      } else {
        showToast('error', 'Terjadi kesalahan di server, silakan coba lagi');
      }
    } else if (error.request) {
      showToast('error', 'Gagal terhubung ke server, periksa koneksi Anda');
    } else {
